...
- An **input and output system** that allows the interaction of the user with the system throguh input and output devices like the keyboard, the mouse, or the screen.
...
So, programming is creating a recipie that the computer will follow to execute what we want to. For example, we may build a program that displays the age of a person in the screen, or prints bill's items and the final price.
...
This boxes are called **variables** and are used in programs to store any data. Variables have different operations like summing or multyplying. Variables are an escential element for programming.

To create a variables we are going to use the keyword `var` followed by it's name.
...
> There are some exceptions to this rules. JavaScript has many syntaxis avaliable, but we are going to work with some habits to make a good programming. For example, the `;` after each sentence is not a must.
...
Looks easy! To shorten the syntaxis we can avrebiate this to steps in just one.
...
The `//` expresion indicates that the words that follows are a **comment** and it's not going to be processed.
...
Functions are a different type od box, magic boxes. Like variables, have a name, but they store _little programs_ inside. So we put some variables inside this box and it returns us a result.
...
Functions is the other main element for programming.
...
**Now with this two tools we can build a simple program:**
